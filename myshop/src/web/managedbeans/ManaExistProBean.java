...
public class ManaExistProBean {
	...
	public void refresh(){
		try {
			...
			indexProItem.add(new SelectItem(indexProList.get(0).getProName(), "产品1(左上)"));
			indexProItem.add(new SelectItem(indexProList.get(1).getProName(), "产品2(中上)"));
			indexProItem.add(new SelectItem(indexProList.get(2).getProName(), "产品3(右侧)"));
			indexProItem.add(new SelectItem(indexProList.get(3).getProName(), "产品4(左下)"));
			indexProItem.add(new SelectItem(indexProList.get(4).getProName(), "产品5(中下)"));
	...
	public String changePro(){
		...
		try {
			if(indexPro.equals(replacePro)){
				throw new Exception("需更换的产品与当前位置所显示的产品相同，请检查");
			}
			Integer proNo = null;
			for (int i = 0; i < 5; i++) {
